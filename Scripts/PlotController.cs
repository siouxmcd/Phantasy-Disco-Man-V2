...
public class PlotController : MonoBehaviour {
    ...
    public static bool isBeat = true;
    ...
    public void updatePlot(List<SpectralFluxInfo> pointInfo, int curIndex = -1)
    {
        ...
        for (int i = windowStart; i < windowEnd; i++)
        {
            ...
            {
                if (plotIndex <= 48 && plotIndex >= 40)
                {
                    DanceBattle.isBeat = true;
                    peakPoint.GetComponentInChildren<MeshRenderer>().material.color = Color.green;
                }
                else if (plotIndex == 39)
                {
                    DanceBattle.isBeat = false;

                    peakPoint.GetComponentInChildren<MeshRenderer>().material.color = Color.red;
                }
                peakPoint.GetComponentInChildren<MeshRenderer>().enabled = true;
            ...
            else
            {
                ...
                peakPoint.GetComponentInChildren<MeshRenderer>().enabled = false;
